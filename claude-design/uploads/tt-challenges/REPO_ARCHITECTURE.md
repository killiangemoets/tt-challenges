...
Nothing under `apps/` exists yet. This is the target tree to implement against.
...
Optional later: npm workspaces so root `npm install` hoists both apps. Not required on day one — two `package.json` files are enough.
...
`docker-compose.yml` today: empty `db`, `minio`, `queue` only. Folders do not have to match compose 1:1.

Typical thin run (document the real path in `DECISIONS.md`):

1. `make up` — backing services
2. Prisma migrate + create bucket + `CreateQueue` (from backend boot or a small init script)
3. Host: `apps/backend` `dev:api` + `dev:worker`, `apps/frontend` Vite (`localhost:5173`; API CORS that origin)

Later: add `api` and `worker` (and optionally `web`) services; keep `make up` as the entry point.
