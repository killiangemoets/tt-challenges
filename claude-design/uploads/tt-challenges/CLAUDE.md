...
| `ARCHITECTURE.md` | Target process map: React SPA, Fastify API, ingest worker behind ElasticMQ, MinIO, Postgres+pgvector |
| `REPO_ARCHITECTURE.md` | Target code layout: `apps/frontend` + `apps/backend` (`api` / `worker` / `common` / `test`) |
| `API_SPECS.md` | HTTP/SSE contract for the Fastify API **and** ingest worker (queue, status, chunk/embed) |
| `DATABASE_SCHEMA.md` | Draft Postgres+pgvector model (lock before first Prisma migration) |
...
- **Respect the rails.** Technology families in STACK.md are fixed (Vite React SPA + Fastify + Prisma + Postgres+pgvector / MinIO+ElasticMQ / Anthropic official SDK + `@xenova/transformers`). No Hono, no Next/SSR, no LangChain/LangGraph. Don't swap frameworks or reach for extra hosted APIs — the only external dependency is the Anthropic API.
